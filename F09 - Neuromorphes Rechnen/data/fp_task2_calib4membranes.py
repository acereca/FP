...
# set up one neuron
# create neurons
neurons = [pynn.Population(1, pynn.IF_facets_hardware1, neuronParams) for i in range(0,192)]
...
meanTime = [np.mean(abs(spiketimes[1:]-spiketimes[:-1])) for spiketimes in spikes]

plt.style.use('bmh')
plt.hist(meanTime, bins=20)
plt.xlabel(r'$\tau_m$ / ms')
plt.ylabel('neurons')
plt.savefig('192neurons.png')

print "{} +- {}".format(np.mean(meanTime), np.std(meanTime))
